...
class RestPointChecklistCategoryController extends Controller
{
    public function index()
    {
        try {
            $categories = RestPointChecklistCategory::withCount('items')
                ->latest()
                ->paginate(15);

            return view('rest-points.checklists.categories.index', compact('categories'));
        } catch (Throwable $th) {
            report($th);
    ...
    public function store(Request $request)
    {
        ...
        try {
            $category = RestPointChecklistCategory::create($validated);

            return redirect()
                ->route('rest-points.checklists.categories.show', $category)
    ...
    public function show(RestPointChecklistCategory $category)
    {
        try {
            $category->load('items');

            return view('rest-points.checklists.categories.show', compact('category'));
        } catch (Throwable $th) {
            report($th);
    ...
    public function update(Request $request, RestPointChecklistCategory $category)
    {
        $validated = $request->validate([
            'name' => ['required', 'string', 'max:255'],
            'is_active' => ['nullable', 'boolean'],
        ...
        try {
            ...
            return redirect()
                ->route('rest-points.checklists.categories.index')
                ->with('success', __('messages.checklist_category_updated') ?? 'Checklist category updated successfully.');
        } catch (Throwable $th) {
            report($th);
    ...
    public function destroy(Request $request, RestPointChecklistCategory $category)
    {
        try {
    ...
    public function toggleStatus(Request $request, RestPointChecklistCategory $category): \Illuminate\Http\JsonResponse
    {
        try {
            $request->validate([
                'is_active' => ['required'],
