    ...
    <div class="card-body">
        ...
        @if($canEdit && !$formationProcess->isValidated())
            <form action="{{ route('formation-processes.save-step', ['formationProcess' => $formationProcess->id, 'stepNumber' => $stepNumber]) }}" method="POST">
                ...
                <div class="mb-3">
                    <label class="form-label">{{ __('messages.budget_approved') }} <span class="text-danger">*</span></label>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="budget_approved" id="budget_approved_yes" value="1" {{ old('budget_approved', $stepData['budget_approved'] ?? '') == '1' ? 'checked' : '' }} required>
