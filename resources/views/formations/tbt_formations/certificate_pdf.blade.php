...
<body>
    <div class="header">
        ...
        <div class="title">{{ __('messages.certificate_of_completion') ?? 'Certificate of Completion' }} - TBT Formation</div>
    ...
    <div class="card">
        ...
        <div class="section">
            <div class="label">{{ __('messages.formation_information') ?? 'Formation Information' }} - TBT Formation</div>
            <table width="100%" cellspacing="0" cellpadding="0" style="border-collapse: collapse; margin-top:6px;">
                <tr>
                    <td class="col">
                        <div class="label">{{ __('messages.tbt_formation_title') ?? 'Title' }}</div>
                        <div class="value">{{ $tbtFormation->title ?? '-' }}</div>
                    </td>
                    <td class="col">
                        <div class="label">{{ __('messages.tbt_formation_year') }}</div>
                        <div class="value">{{ $tbtFormation->year }}</div>
                    </td>
                </tr>
                <tr>
                    <td class="col">
                        <div class="label">{{ __('messages.tbt_formation_month') }}</div>
                        <div class="value">{{ \Carbon\Carbon::create($tbtFormation->year, $tbtFormation->month, 1)->locale('fr')->monthName }}</div>
                    </td>
                    <td class="col">
                        <div class="label">{{ __('messages.tbt_formation_week') }}</div>
                        <div class="value">{{ \Carbon\Carbon::parse($tbtFormation->week_start_date)->format('d/m') }} - {{ \Carbon\Carbon::parse($tbtFormation->week_end_date)->format('d/m') }}</div>
                    </td>
                </tr>
                <tr>
                    <td class="col">
                        <div class="label">{{ __('messages.formation_participant') }}</div>
                        <div class="value">{{ $tbtFormation->participant ?? '-' }}</div>
                    </td>
                    <td class="col">
                        <div class="label">{{ __('messages.status') }}</div>
                        <div class="value">{{ $driverFormation->status === 'done' ? __('messages.completed') : ($driverFormation->status ?? '-') }}</div>
                    </td>
                </tr>
            </table>
        </div>
