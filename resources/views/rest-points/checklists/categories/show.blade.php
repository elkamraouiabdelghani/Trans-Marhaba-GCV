        ...
        <div class="d-flex justify-content-between align-items-center mb-4 bg-white p-4 rounded-3 shadow-sm">
            <div class="d-flex align-items-center gap-3">
                ...
                <span class="badge {{ $category->is_active ? 'bg-success' : 'bg-secondary' }}">
                    {{ $category->is_active ? (__('messages.active') ?? 'Active') : (__(
                        'messages.inactive') ?? 'Inactive') }}
                </span>
            </div>
            <a href="{{ route('rest-points.checklists.categories.index') }}" class="btn btn-outline-secondary btn-sm">
                                            ...
                                            <td class="text-end pe-3">
                                                <div class="btn-group" role="group">
                                                    <button type="button" 
                                                            ...
                                                            data-update-url="{{ route('rest-points.checklists.items.update', $item) }}"
                                                    ...
                                                    <button type="button"
                                                            ...
                                                            data-delete-url="{{ route('rest-points.checklists.items.destroy', $item) }}"
    ...
    <div class="modal fade" id="createChecklistItemModal" tabindex="-1" aria-labelledby="createChecklistItemModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('rest-points.checklists.items.store') }}">
                    @csrf
                    <input type="hidden" name="rest_points_checklist_category_id" value="{{ $category->id }}">
    ...
    @push('scripts')
        <script>
            ...
            if (deleteCategoryModal) {
                deleteCategoryModal.addEventListener('show.bs.modal', function (event) {
                    ...
                    if (deleteCategoryForm && categoryId) {
                        deleteCategoryForm.action = `/rest-points-checklist-categories/${categoryId}`;
                    }
            ...
            function performToggleCategoryStatus(categoryId, isActive) {
                fetch(`/rest-points-checklist-categories/${categoryId}/toggle-status`, {
                    method: 'POST',
                    headers: {
