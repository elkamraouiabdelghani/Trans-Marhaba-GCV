<x-app-layout>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-10 col-xl-10">
                <div class="card border-0 shadow-sm">
                    <form action="{{ route('journeys.store') }}" method="POST" id="createJourneyForm" enctype="multipart/form-data">
                        @csrf
                        <div class="card-body p-4">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="hidden" name="from_latitude" id="create_from_latitude" value="{{ old('from_latitude') }}" required>
                                    <input type="hidden" name="from_longitude" id="create_from_longitude" value="{{ old('from_longitude') }}" required>
                                    <input type="hidden" name="from_location_name" id="create_from_location_name_hidden" value="{{ old('from_location_name') }}">
                                    <small class="text-muted d-block mt-1" id="create-from-coordinates-label">
                                        ...
                                        @else
                                            {{ __('messages.location_map_help') ?? 'Use address/location/coordinates or the map to set coordinates.' }}
                                        @endif
                                    </small>
                                    @error('from_latitude')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <!-- Hidden fields and resolved coordinates label -->
                                    <input type="hidden" name="to_latitude" id="create_to_latitude" value="{{ old('to_latitude') }}" required>
                                    <input type="hidden" name="to_longitude" id="create_to_longitude" value="{{ old('to_longitude') }}" required>
                                    <input type="hidden" name="to_location_name" id="create_to_location_name_hidden" value="{{ old('to_location_name') }}">
                                    <small class="text-muted d-block mt-1" id="create-to-coordinates-label">
                                ...
                                <div class="col-12">
                                    <label for="create_details" class="form-label fw-semibold">
                                        {{ __('messages.details') ?? 'Details' }}
                                    </label>
                                    <textarea class="form-control @error('details') is-invalid @enderror"
                                              id="create_details" name="details" rows="4">{{ old('details') }}</textarea>
                                    @error('details')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <div id="blackPointsContainer">
                                        <!-- Black points will be added here dynamically -->
                                    </div>
                                </div>
                            </div>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-center align-middle">
                                                                <input type="number" 
                                                                       ...
                                                                       min="1" 
                                                                       max="10" 
                                                                       value="{{ old("checklist.{$item->id}.weight", 1) }}"
                                                            ...
                                                            <td class="text-center align-middle">
                                                                <input type="number" 
                                                                       ...
                                                                       min="1" 
                                                                       max="5" 
                                                                       value="{{ old("checklist.{$item->id}.score", 1) }}"
                                                                       data-item-id="{{ $item->id }}">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <!-- From Location Name -->
                    <div class="mt-3">
                        <label for="create_from_location_name" class="form-label fw-semibold">
                        ...
                        <input type="text" 
                               ...
                               id="create_from_location_name" 
                               value="{{ old('from_location_name') }}"
    ...
    <!-- Add Black Point Modal (for create form) -->
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <!-- To Location Name -->
                    <div class="mt-3">
                        <label for="create_to_location_name" class="form-label fw-semibold">
                        ...
                        <input type="text" 
                               ...
                               id="create_to_location_name" 
                               value="{{ old('to_location_name') }}"
                               placeholder="{{ __('messages.location_name_placeholder') ?? 'e.g., Casablanca, Rabat, etc.' }}">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        waitForLeaflet(function() {
            ...
            function setFromCoordinates(lat, lng, centerMap = true) {
                ...
                const hiddenLat = document.getElementById('create_from_latitude');
                const hiddenLng = document.getElementById('create_from_longitude');
                ...
                // Update label
                const formLabel = document.getElementById('create-from-coordinates-label');
                if (formLabel) {
            ...
            function setToCoordinates(lat, lng, centerMap = true) {
                ...
                const hiddenLat = document.getElementById('create_to_latitude');
                const hiddenLng = document.getElementById('create_to_longitude');
                if (hiddenLat) hiddenLat.value = lat.toFixed(6);
                if (hiddenLng) hiddenLng.value = lng.toFixed(6);
                ...
                // Update label
                const formLabel = document.getElementById('create-to-coordinates-label');
                if (formLabel) {
                    const coordsLabel = @json(__('messages.location_coords_label') ?? 'Coordinates');
                    formLabel.innerHTML = coordsLabel + ': <span class="fw-semibold">' + lat.toFixed(6) + ', ' + lng.toFixed(6) + '</span>';
                }
            ...
            if (fromLocationModal) {
                fromLocationModal.addEventListener('shown.bs.modal', function () {
                    ...
                    if (!fromMap) {
                        ...
                        const existingLat = document.getElementById('create_from_latitude')?.value;
                        const existingLng = document.getElementById('create_from_longitude')?.value;
            ...
            if (toLocationModal) {
                toLocationModal.addEventListener('shown.bs.modal', function () {
                    ...
                    if (!toMap) {
                        ...
                        // Load existing coordinates if any
                        const existingLat = document.getElementById('create_to_latitude')?.value;
                        const existingLng = document.getElementById('create_to_longitude')?.value;
                        if (existingLat && existingLng) {
                            const lat = parseFloat(existingLat);
                            const lng = parseFloat(existingLng);
                            toMap.setView([lat, lng], 13);
        ...
        document.getElementById('confirmFromLocation')?.addEventListener('click', function () {
            if (fromLat !== null && fromLng !== null) {
                ...
                checkAndEnableBlackPointButton();
            }
            // Sync location name from modal to hidden input
            const modalLocationName = document.getElementById('create_from_location_name')?.value || '';
            const hiddenLocationName = document.getElementById('create_from_location_name_hidden');
        ...
        document.getElementById('confirmToLocation')?.addEventListener('click', function () {
            ...
            // Sync location name from modal to hidden input
            const modalLocationName = document.getElementById('create_to_location_name')?.value || '';
            const hiddenLocationName = document.getElementById('create_to_location_name_hidden');
            if (hiddenLocationName) {
                hiddenLocationName.value = modalLocationName;
            }
        });

        // Check and enable black point button
        function checkAndEnableBlackPointButton() {
            const fromLat = document.getElementById('create_from_latitude')?.value;
            const fromLng = document.getElementById('create_from_longitude')?.value;
            const toLat = document.getElementById('create_to_latitude')?.value;
            const toLng = document.getElementById('create_to_longitude')?.value;
            const btn = document.getElementById('addBlackPointBtn');
            
            if (btn && fromLat && fromLng && toLat && toLng) {
                btn.disabled = false;
            } else if (btn) {
                btn.disabled = true;
            }
        ...
        // Check on page load if old values exist
        checkAndEnableBlackPointButton();
        ...
        let blackPointIndex = 0;
            ...
            if (addBlackPointModal) {
                ...
                const fromLat = parseFloat(document.getElementById('create_from_latitude')?.value);
                const fromLng = parseFloat(document.getElementById('create_from_longitude')?.value);
                const toLat = parseFloat(document.getElementById('create_to_latitude')?.value);
                const toLng = parseFloat(document.getElementById('create_to_longitude')?.value);

                if (!fromLat || !fromLng || !toLat || !toLng) {
                    alert(@json(__('messages.location_required') ?? 'Please select both from and to locations first.'));
                    closeModalSafely('addBlackPointModal');
                    return;
                }
            ...
            addBlackPointModal.addEventListener('hidden.bs.modal', function () {
                ...
                document.getElementById('black_point_icon_type_input').value = 'danger';
        ...
        // Button now uses data-bs-toggle, so we don't need manual click handler
        // Map initialization is handled in the show.bs.modal event listener below
        ...
        // Handle remove buttons for existing black points (if any from old input)
