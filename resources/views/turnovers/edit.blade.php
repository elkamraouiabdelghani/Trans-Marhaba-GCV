<x-app-layout>
    ...
    <div class="container-fluid py-4">
        ...
        <div class="card border-0 shadow-sm col-md-10 mx-auto">
            <div class="card-header bg-white border-bottom py-3">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 text-dark fw-bold">
                        ...
                        {{ __('messages.edit_turnover') ?? 'Modifier le Turnover' }}: #{{ $turnover->position }}
                    </h5>
            ...
            <div class="card-body p-4">
                <form action="{{ route('turnovers.update', $turnover) }}" method="POST" id="turnoverForm">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6 mb-3">
                            <label class="form-label">{{ __('messages.person_type') ?? 'Type de personne' }} <span class="text-danger">*</span></label>
                            <div class="btn-group w-100" role="group">
                                <input type="radio" class="btn-check" name="person_type" id="person_type_driver" value="driver" {{ $turnover->driver_id ? 'checked' : '' }} disabled>
                                <label class="btn btn-outline-dark" for="person_type_driver">
                                    <i class="bi bi-car-front me-1"></i>
                                    {{ __('messages.driver') ?? 'Chauffeur' }}
                                </label>
                                <input type="radio" class="btn-check" name="person_type" id="person_type_admin" value="admin" {{ $turnover->user_id ? 'checked' : '' }} disabled>
                                <label class="btn btn-outline-dark" for="person_type_admin">
                                    <i class="bi bi-person-gear me-1"></i>
                                    {{ __('messages.administration') ?? 'Administration' }}
                                </label>
                            </div>
                        </div>
                        <div class="col-md-6 mb-3">
                            <!-- Driver Selection -->
                            <div id="driver_section" class="person-section" style="display: {{ $turnover->driver_id ? 'block' : 'none' }};">
                                <div class="row">
                                    ...
                                    <select class="form-select @error('driver_id') is-invalid @enderror" 
                                        ...
                                        @foreach($drivers as $driver)
                                            <option value="{{ $driver->id }}" 
                                                    ...
                                                    {{ old('driver_id', $turnover->driver_id) == $driver->id ? 'selected' : '' }}>
                                                {{ $driver->full_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <input type="hidden" name="driver_id" value="{{ $turnover->driver_id }}">
                                    @error('driver_id')
                            ...
                            <!-- User Selection -->
                            <div id="admin_section" class="person-section" style="display: {{ $turnover->user_id ? 'block' : 'none' }};">
                                <div class="row">
                                    ...
                                    <select class="form-select @error('user_id') is-invalid @enderror" 
                                            ...
                                            name="user_id"
                                            disabled>
                                        <option value="">{{ __('messages.select_user') ?? 'Sélectionner un membre de l\'administration' }}</option>
                                        @foreach($users as $user)
                                            <option value="{{ $user->id }}" {{ old('user_id', $turnover->user_id) == $user->id ? 'selected' : '' }}>
                                                {{ $user->name }} ({{ $user->role ?? 'N/A' }})
                                            </option>
                                        @endforeach
                                    </select>
                                    <input type="hidden" name="user_id" value="{{ $turnover->user_id }}">
                                    @error('user_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                    ...
                    <h6 class="text-primary mb-3 mt-4 border-bottom pb-2">
                        ...
                        {{ __('messages.turnover_information') ?? 'Informations du turnover' }}
                    </h6>
                    <div class="row mb-4">
                        <div class="col-md-6 mb-3">
                            <label for="departure_date" class="form-label">{{ __('messages.departure_date') ?? 'Date de départ' }} <span class="text-danger">*</span></label>
                            <input type="date" 
                                   class="form-control @error('departure_date') is-invalid @enderror" 
                                   id="departure_date" 
                                   name="departure_date" 
                                   value="{{ old('departure_date', $turnover->departure_date ? $turnover->departure_date->format('Y-m-d') : '') }}" 
                                   readonly
                                   required>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   id="flotte" 
                                   name="flotte" 
                                   value="{{ old('flotte', $turnover->flotte) }}" 
                                   readonly
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="position" class="form-label">{{ __('messages.position') ?? 'Poste' }}</label>
                            <input type="text" 
                                   class="form-control @error('position') is-invalid @enderror" 
                                   id="position" 
                                   name="position" 
                                   value="{{ old('position', $turnover->position) }}" 
                                   readonly
                                   placeholder="{{ __('messages.auto_filled') ?? 'Rempli automatiquement' }}">
                            @error('position')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        {{-- confirmed at --}}
                        <div class="col-md-6 mb-3">
                            <label for="confirmed_at" class="form-label">{{ __('messages.confirmed_at') }}</label>
                            <input type="text" 
                                   class="form-control @error('confirmed_at') is-invalid @enderror" 
                                   id="confirmed_at" 
                                   name="confirmed_at" 
                                   value="{{ old('confirmed_at', $turnover->confirmed_at ? $turnover->confirmed_at->format('d/m/Y') : '') }}" 
                                   readonly>
                            @error('confirmed_at')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <h6 class="text-primary mb-3 mt-4 border-bottom pb-2">
                        ...
                        {{ __('messages.departure_details') ?? 'Détails du départ' }}
                    </h6>
                    <div class="row mb-4">
                        <div class="col-12 mb-3">
                            <label for="departure_reason" class="form-label">{{ __('messages.departure_reason') ?? 'Raison de départ' }} <span class="text-danger">*</span></label>
                            <textarea class="form-control @error('departure_reason') is-invalid @enderror" 
                                      id="departure_reason" 
                                      name="departure_reason" 
                                      rows="4" 
                                      required>{{ old('departure_reason', $turnover->departure_reason) }}</textarea>
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea class="form-control @error('interview_notes') is-invalid @enderror" 
                                      ...
                                      rows="4">{{ old('interview_notes', $turnover->interview_notes) }}</textarea>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   id="interviewed_by" 
                                   name="interviewed_by" 
                                   value="{{ old('interviewed_by', $turnover->interviewed_by) }}">
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea class="form-control @error('observations') is-invalid @enderror" 
                                      id="observations" 
                                      name="observations" 
                                      rows="4">{{ old('observations', $turnover->observations) }}</textarea>
                            @error('observations')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const isConfirmed = {{ $turnover->isConfirmed() ? 'true' : 'false' }};
