<div class="card border-0 shadow-sm">
    <div class="card-header bg-white border-0 py-3">
        <h5 class="mb-0 text-dark fw-bold">
            ...
            {{ __('messages.step_3_planification') }}
        </h5>
    </div>
    <div class="card-body">
        @php
        ...
        @if(!$isRejected)
            <form action="{{ route('changements.save-step', ['changement' => $changement->id, 'stepNumber' => $stepNumber]) }}" method="POST" data-validate-form>
                ...
                <div class="mb-3">
                    <label for="change_planning" class="form-label">
                        {{ __('messages.change_planning') }} <span class="text-danger">*</span>
                    </label>
                    <textarea class="form-control @error('change_planning') is-invalid @enderror" 
                              id="change_planning" 
...
>{{ old('change_planning', $stepData['change_planning'] ?? '') }}</textarea>
                    ...
                    @enderror
                    <small class="form-text text-muted">{{ __('messages.change_planning_help') }}</small>
                </div>

                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label for="planned_by" class="form-label">{{ __('messages.planned_by') }}</label>
                        <input type="text" 
                               class="form-control @error('planned_by') is-invalid @enderror" 
                               id="planned_by" 
                               name="planned_by" 
...
>
                    ...
                    <div class="col-md-6 mb-3">
                        <label for="planning_date" class="form-label">{{ __('messages.planning_date') }}</label>
                        <input type="date" 
                               class="form-control @error('planning_date') is-invalid @enderror" 
                               id="planning_date" 
                               name="planning_date" 
...
>
                ...
                <div class="mb-3">
                    <label for="planning_notes" class="form-label">{{ __('messages.notes') }} ({{ __('messages.optional') }})</label>
                    <textarea class="form-control @error('planning_notes') is-invalid @enderror" 
                              id="planning_notes" 
                              name="planning_notes" 
                              rows="3"
                              {{ !$canEdit ? 'readonly' : '' }}>{{ old('planning_notes', $stepData['planning_notes'] ?? '') }}</textarea>
                    @error('planning_notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
        ...
        @else
            <div class="row">
                <div class="col-12 mb-3">
                    <label class="form-label fw-bold">{{ __('messages.change_planning') }}</label>
                    <p class="text-muted">{{ $stepData['change_planning'] ?? __('messages.not_available') }}</p>
                </div>
            </div>
